...
BOT_TOKEN = '8542517831:AAGop3lGd5cRmMUZneu4I8yO1gwBSwNsV4A'
OWNER_ID = 8722144519
BOT_USERNAME = '@GETO_X_HACKER_BOT'  # Change to your bot's username
...
if 'raid_lines' not in data:
    data['raid_lines'] = ["MADARCHOD SALE KITNA ROYEGA?", "TERI MA KI CHUT 🥵", "BHOSDIWALE", "LAUDE LAG GAYE?", "RANDI KE BACHCHE",  "TERI MAKI CHUT MADARCHODO HIZDA HAI HAI TUM MADARCHODO BOL DE YUTA TERA BAAP HAI WARNA TERI KI CHUT KOI RAMDI KI AULAD NHI BACHA PAYEGA AAJ SAMJH LE MADRCHOD",
...
# ==================== ABUSE REPLY FOR UNAUTHORIZED ====================
ABUSE_LINES = [
    "TERI MA KI CHUT 🥵 BC MADARCHOD SE PEHLE OWNER BAN",
    "BHAI TERA BAP HAI KYO USE KAR RAHA HAI LAUDEEE 🤬",
    "Bhosdiwale Owner se permission le pehle gandu",
    "Randi ke bache tera baap bhi use nahi kar sakta 🤣",
    "Madarchod kitna bhi koshish kar le owner nhi ban sakta 🫵😂",
    "Sale gareeb Tera koi kaam nhi hai 🗿🫵",  "TERI MAKI CHUT MADARCHODO HIZDA HAI HAI TUM MADARCHODO BOL DE YUTA TERA BAAP HAI WARNA TERI KI CHUT KOI RAMDI KI AULAD NHI BACHA PAYEGA AAJ SAMJH LE MADRCHOD",
    "AIR JORDEN KE JUTE SE TERI KI CHUT PR MAAR MAAR KE LAAL KR DUNGA KALI SE LAAL 😋🥵 RANDI MADARCHODO",
    "MADARCHODO BAAP SE LADEGA APNE TERI MA KI CHUT KHA JAUNGA RAMDI",
    "TERI MAA KI CHHUT KA KHAA JAUNGAA MADARCHODO RANDI KI AULAD SPAM KARTA HAI MADARCOD KE CHAKKE KI AULAD TERE BAAP KO GADHE KE LAND SEE CHODUNGA",
    "TERI MAKI CHUT MADARCHOD SUR KE LAND SE MA CHODUNGA GANDI CHUT KI KALI AULAD FATI KALI CHUT KI AULD CHAMR TERI MAIYA KI CHUT KO CHOR BAZR ME BECHUNGA MADRACHOD",
...
async def send_abuse(event):
    """Send 4-6 abuse replies to unauthorized users"""
    count = random.randint(4, 6)
    lines = random.sample(ABUSE_LINES, min(count, len(ABUSE_LINES)))
    for line in lines:
        try:
        ...
        if event.user_added and event.users:
            if me.id in event.users:
                ...
                if chat_id not in groups:
                    ...
                    save_data()
                    try:
                        await event.reply("✅ Bot is active! Use /menu for commands.")
                    except:
                        pass
...
async def callback_handler(event):
    ...
    if data_cb == "my_owner":
        ...
        text = (
            ...
            "`.rrr @user` - Ultra fast raid\n"
            "`.s` - Stop raid\n"
            "`.addline <text>` - Add raid line\n"
            "`.lines` - Show all raid lines\n\n"
            "**🔇 Mute:**\n"
            "`.mute @user [rea son]` - Mute user\n"
        ...
        await event.edit("➕ **Add me to any group and I'll automatically track it!**\n\nOr use /menu in the target group to register it.")
    
    elif data_cb == "chat_box":
        await event.edit("💬 **Chat Box Active!**\n\nSend your message and I'll forward it to the owner.")
...
# ==================== RAID SYSTEM ====================
async def raid_worker(event, target_user, target_id, target_name):
    ...
    while raid_active.get(event.chat_id, False) and raid_active.get('target') == target_id:
        try:
            async with raid_semaphore:
                ...
                mention = f"<a href='tg://user?id={target_id}'>{target_name}</a> {line}"
                await bot.send_message(event.chat_id, mention, parse_mode='html')
                await asyncio.sleep(0.08)  # ~12 msg/sec per worker
...
async def raid_cmd(event):
    global raid_active, raid_target
    if not bot_alive:
        await event.reply("❌ Bot is offline!")
        return
    if not is_authorized(event.sender_id):
        await send_abuse(event)
        return
    ...
    workers = [raid_worker(event, entity, target_id, target_name) for _ in range(3)]
...
async def ultra_raid_cmd(event):
    ...
    workers = [raid_worker(event, entity, target_id, target_name) for _ in range(5)]
    await asyncio.gather(*workers)
...
async def mute_cmd(event):
    ...
    target_username = event.message.text.split()[1].lstrip('@')
    reason = ' '.join(event.message.text.split()[2:]) if len(event.message.text.split()) > 2 else 'No reason'
    ...
    await event.reply(f"🔇 **Muted** <a href='tg://user?id={target_id}'>User</a>\nReason: {reason}", parse_mode='html')
    
    # Auto-delete muted user messages
    @bot.on(events.NewMessage(chats=chat_id))
    async def auto_delete_muted(e):
        if e.sender_id in muted_users.get(chat_id, {}):
            try:
                await asyncio.sleep(0.5)  # <1 sec
                await e.delete()
            except:
                pass
...
async def unmute_cmd(event):
    ...
    if chat_id in muted_users and target_id in muted_users[chat_id]:
        ...
        await event.reply(f"🔊 **Unmuted** <a href='tg://user?id={target_id}'>User</a>", parse_mode='html')
    else:
...
async def shayari_cmd(event):
    ...
    if len(parts) > 2 and '@' in parts[2]:
        ...
        for i in range(min(count, 20)):
            ...
            mention = f"<a href='tg://user?id={target_id}'>{shayari_type}</a> {line}"
            await bot.send_message(event.chat_id, mention, parse_mode='html')
            await asyncio.sleep(0.3)
    else:
        # Adding shayari
...
async def sticker_cmd(event):
    ...
    for i in range(min(count, 10)):
        try:
            ...
            mention = f"<a href='tg://user?id={target_id}'>Sticker</a>"
            await bot.send_message(event.chat_id, mention, parse_mode='html')
...
async def sudo_cmd(event):
    ...
    if target_id not in sudo_users:
        ...
        save_data()
        await event.reply(f"✅ **Sudo added!** <a href='tg://user?id={target_id}'>User</a>", parse_mode='html')
    else:
        await event.reply("⚠️ User is already sudo!")
...
async def remsudo_cmd(event):
    ...
    if target_id in sudo_users:
        ...
        data['sudo_users'] = sudo_users
        save_data()
        await event.reply(f"✅ **Sudo removed!** <a href='tg://user?id={target_id}'>User</a>", parse_mode='html')
    else:
...
async def info_cmd(event):
    ...
    try:
        ...
        text = (
            ...
            f"**Scam:** {'Yes' if getattr(user, 'scam', False) else 'No'}\n"
...
async def hack_cmd(event):
    ...
    hack_lines = [
        ...
        f"🟡 [4/5] Accessing camera and microphone... 📷🎤",
        f"🟢 [5/5] Downloading complete! ✅",
        f"💀 **{target_name} has been HACKED!**\n\n📱 Device: {random.choice(['iPhone 15', 'Samsung S24', 'OnePlus 12', 'Google Pixel 8'])}\n📍 Location: {random.choice(['Delhi, India', 'Mumbai, India', 'Bangalore, India', 'Pune, India'])}\n📡 IP: {random.randint(1,255)}.{random.randint(1,255)}.{random.randint(1,255)}.{random.randint(1,255)}\n🔑 Password: {'*' * random.randint(6,12)}"
    ]
...
# ==================== SPAM ====================
@bot.on(events.NewMessage(pattern=r'^[.!/]spam\s+@?'))
async def spam_cmd(event):
    ...
    parts = event.message.text.split()
    if len(parts) < 3:
        await event.reply("❌ Usage: .spam @user <count>")
        return
    
    target_username = parts[1].lstrip('@')
    count = int(parts[2]) if parts[2].isdigit() else 5
    ...
    await event.reply(f"✉️ **Spamming** {target_name} **{count} times...**", parse_mode='markdown')
    
    for i in range(min(count, 50)):
        mention = f"<a href='tg://user?id={target_id}'>{target_name}</a> SPAM #{i+1}"
        await bot.send_message(event.chat_id, mention, parse_mode='html')
        await asyncio.sleep(0.3)
...
async def restart_cmd(event):
    ...
    # Save state and exit - Render will auto-restart
    save_data()
    os._exit(0)
...
async def forward_cmd(event):
    ...
    for i in range(min(count, 10)):
        try:
            mention = f"<a href='tg://user?id={target_id}'>Forward #{i+1}</a>"
            await bot.send_message(event.chat_id, mention, parse_mode='html')
...
async def unauthorized_handler(event):
    """Catch-all for any command not handled above"""
    if not is_authorized(event.sender_id):
        await send_abuse(event)
...
async def main():
    ...
    print(f"📝 Raid lines: {len(data.get('raid_lines', []))}")
